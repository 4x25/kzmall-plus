...
响应外层包含 `status`、`msg`、`totalAmount`、`outAmount`、`payment1`、`Amount`、`data` 和 `editDesc`；列表行为 `data.rows[]`。

| 响应字段/路径 | 类型/可空 | 表格列 | 格式/单位 | 释义 | 证据 |
|---|---|---|---|---|---|
| `contactName` | string/可空 | 客户名称 | 文本 | 客户显示名称 | 非空响应 + 表格 `prop` |
| `billNo` | string | 销售单号 | 单号 | 销售单唯一业务编号 | 非空响应 + 表格 `prop` |
| `activity_id` | string/number/可空 | 活动 ID | ID | 关联 E 站活动 | 非空响应 + scoped slot |
| `createTime` | string | 开单时间 | 日期时间 | 销售单创建时间 | 非空响应 + 表格 `prop` |
| `storeName` | string/可空 | 销售门店 | 文本 | 开单门店 | 非空响应 + 表格 `prop` |
| `totalAmount` | numeric string/number | 订单金额 | 元 | 优惠前订单金额 | 非空响应 + 表格 `prop` |
| `amount` | numeric string/number | 出库金额 | 元 | 已出库对应金额 | 非空响应 + 表格 `prop` |
| `rpAmount` | numeric string/number | 已收金额 | 元 | 已收款金额 | 非空响应 + 表格 `prop` |
| `disAmount` | numeric string/number | 优惠金额 | 元 | 整单优惠金额 | 非空响应 + 表格 `prop` |
| `disRate` | numeric string/number | 优惠率 | `%` | 整单优惠比例 | 非空响应 + 表格 `prop` |
| `billStatusName` | string | 订单状态 | 文本 | 后端状态显示名 | 非空响应 + scoped slot |
| `hxStateCodeName` | string/可空 | 收款状态 | 文本 | 收款/核销状态显示名 | 表格 `prop`；类型待更多响应复核 |
| `billNo_typeName` | string | 订单类型 | 文本 | 销售、铺货等显示名 | 非空响应 + 表格 `prop` |
| `payTypeName` | string | 收款方式 | 文本 | 挂账/现金显示名 | 非空响应 + 表格 `prop` |
| `way` | string/可空 | 结算方式 | 文本 | 结算方式显示名 | 非空响应 + 表格 `prop` |
| `printing_times` | integer/string | 打印次数 | 次 | 打印计数 | 非空响应 + 表格 `prop` |
| `tyQuantity` | integer/string | 出库次数 | 次 | 出库操作计数 | 非空响应 + 表格 `prop` |
| `salesName` | string/可空 | 销售员 | 文本 | 销售人员名称 | 非空响应 + 表格 `prop` |
| `userName` | string/可空 | 开单员 | 文本 | 制单人员名称 | 非空响应 + 表格 `prop` |
| `sourceTypeName` | string/可空 | 数据来源 | 文本 | 订单来源显示名 | 非空响应 + 表格 `prop` |
| `delieverName` | string/可空 | 送货员 | 文本 | 送货人员名称 | 非空响应 + 表格 `prop` |
| `vins` | string/array-like/可空 | 关联 VIN 码 | 文本 | 一个或多个关联 VIN | 非空响应 + 表格 `prop` |
| `elapsedTime` | string/number/可空 | 消耗时间 | 页面格式 | 从创建到当前业务节点的耗时 | 表格 `prop`；类型待更多响应复核 |
| `description` | string/可空 | 备注 | 文本 | 整单备注 | 非空响应 + 表格 `prop` |
...
| 撤销时间 | `cancel_beg_time` / `cancel_end_time` | datetime/date string | 空 | 撤销时间范围 | 高级筛选绑定 |
...
响应外层为 `records:string`、`total:number`、`status:number`、`msg:string`、`data:array`、`totalAmount:object`、`editDesc:number`。列表直接位于 `data[]`，不是 `data.rows[]`。
...
响应外层为 `success:boolean`、`status:string`、`redirect:null`、`msg:string`、`data:object`；`data.page:number`、`data.records:string`、`data.total:number`、`data.rows:array`。`data.rows[]` 使用下表字段：
...
有效报价响应外层为 `status:number`、`msg:string`、`records:number`、`total:number`、`rows:array`、`kzSwitch:string`。
...
等待报价当前响应外层为 `success`、`status`、`redirect`、`msg`、`data`，其中 `data.total:number`、`data.records:number`，当前列表为空；作废报价外层与有效报价相同但 `rows[]` 为空。因此两者的行字段类型/可空性仍按 B 级处理。

## 微仓出入库

证据等级：B。真实空表单查询、响应外层、单据类型枚举和全部表格 `prop` 已确认；默认日期范围结果为空，行字段类型/可空性待非空响应补证。
...
主查询响应为 `success:boolean`、`status:string`、`msg:string`、`data.list[]`、`data.records:number`。当前 `data.list` 为空；列契约如下：
...
无独立查询条件。响应为 `success:boolean`、`status:string`、`msg:string`、`data.list[]`；当前 `list` 为空。
...
| 品类品牌 | `brandIds` | string | 逗号分隔 | 所选品牌 ID 去重后拼接 | `getCategoryAndBrand()` |
| 品类品牌 | `cateBrandMap` | JSON string | 例如 `{<cateCode>:[<brandId>]}` | 保留每个品类与其品牌的组合关系 | `getCategoryAndBrand()` |
...
| 补货时间 | `beginDate` / `endDate` | datetime string | 必选 | 补货/领用统计区间；开始时间可能被上次补货时间校正 | 日期控件 + `search()` |
| 定点商品 | `skuId` | string/number | 普通查询为空 | `source=2` 时指定要刷新的 SKU | `search()` |
| 分类 | `categoryIds` | string | 可空，组件集合编码 | 所选商品分类 | 分类组件 `fixVal()` |
...
| 品类品牌 | `brand_id` | string | 逗号分隔 | 所选品牌 ID | `getRecommendParams()` |
| 品类品牌 | `cate_brand_map` | JSON string | 品类到品牌数组的映射 | 保留品类与品牌组合关系 | `getRecommendParams()` |
...
| 分类 | `category_ids` | string | 可空，组件集合编码 | 所选商品分类 | 分类组件 `fixVal()` |
...
| UI 筛选项 | 请求参数 | 类型 | 默认值/枚举 | 释义 | 证据 |
|---|---|---|---|---|---|
| 套餐名称 | `bag_name` | string | 空 | 按套餐名称过滤 | 组件 `v-model` |
| 状态 | `status` | number/string | 空全部；`1` 在售中、`2` 已下架 | 套餐上下架状态 | 组件枚举 |
| 来源 | `source_type` | number | `2` | 当前单站 E 站来源固定值 | 抓包 + 查询对象 |
| 页码/页大小 | `page` / `limit` | integer | `1` / `20` | 分页 | 抓包 + 查询对象 |

响应外层：`code`、`message`、`data.count`、`data.page_count`、`data.list[]`、`data.select_status[]`。
...
| 上线时间 | `online_time` | array | 空数组时省略；`[开始, 结束]` | 上线日期范围 | 组件 watcher |
| 开始时间 | `begin_time` | array | 空数组时省略；`[开始, 结束]` | 活动开始日期范围 | 组件 watcher |
| 结束时间 | `end_time` | array | 空数组时省略；`[开始, 结束]` | 活动结束日期范围 | 组件 watcher |
| 页码/页大小 | `page` / `limit` | integer | `1` / `20` | 分页 | 抓包 |

响应列表位于 `data.data[]`，总数为 `data.records`，不是 `data.list[]`。
...
旧文档中的查询参数 `status` 不存在；状态只作为返回列展示。响应包含 `code`、`message`、`data.count`、`data.page_count`、`data.list[]`。
...
| 活动平台 | `activity_source_type` | string/number | 空全部 | 活动来源平台 | 组件 `v-model` |
| 订单状态 | `order_status` | number/string | `-1` 全部 | 订单状态值 | 组件默认值 + 抓包 |
| 下单时间 | `begin_time` / `end_time` | datetime string | 空 | 订单时间范围 | 组件 watcher |
| 是否套包 | `is_bag` | number/string | 空全部 | 套包订单标志 | 组件 `v-model` |
| 支付方式 | `pay_mode` | number/string | 空全部 | 支付方式值 | 组件 `v-model` |
| 数据来源 | `data_source` | number | `1` E 站小程序 | 订单数据来源 | 组件默认值 + 抓包 |
| 全量导出标志 | `is_all` | number | 查询固定 `0` | `1` 仅用于导出，不属于查询 Agent | 查询对象 |
| 页码/页大小 | `page` / `limit` | integer | `1` / `20` | 分页 | 抓包 |

| 响应字段/路径 | 类型/可空 | 表格列 | 格式/单位 | 释义 | 证据 |
|---|---|---|---|---|---|
| `order_no` | string | 订单编号 | 单号 | E 站订单号 | 非空响应 + 组件 `prop` |
| `sa_invoice_order_no` | string/可空 | 销售单号 | 单号 | 生成的销售单号 | 非空响应 + 组件 `prop` |
| `create_time` | string | 订单时间 | 日期时间 | 下单时间 | 非空响应 + 组件 `prop` |
| `activity_id` | number/string/可空 | 活动编号 | ID | 关联活动 ID | 非空响应 + 组件 `prop` |
| `activity_name` | string/可空 | 活动名称 | 文本 | 关联活动名称 | 非空响应 + 组件 `prop` |
| `activity_source_type_name` | string/可空 | 活动平台 | 文本 | 活动来源显示名 | 非空响应 + 组件 `prop` |
| `is_bag` | number/string/可空 | 是否套包 | 布尔显示 | 套包订单标志 | 组件渲染；值域待更多响应复核 |
| `pay_mode_name` | string/可空 | 支付方式 | 文本 | 支付方式显示名 | 非空响应 + 组件 `prop` |
| `source_name` | string | 订单来源 | 文本 | 来源显示名 | 非空响应 + 组件 `prop` |
| `contact_name` | string | 维修厂名称 | 文本 | 下单客户名称 | 非空响应 + 组件 `prop` |
| `order_status_name` | string | 订单状态 | 文本 | 状态显示名 | 非空响应 + 组件 `prop` |
| `pay_time` | string/可空 | 付款时间 | 日期时间 | 支付完成时间 | 非空响应 + 组件 `prop` |
| `activity_amount` | number/string | 优惠金额 | 元 | 活动优惠金额 | 非空响应 + 组件 `prop` |
| `coupon_source_type_name` | string/可空 | 优惠券来源 | 文本 | 优惠券来源显示名 | 非空响应 + 组件 `prop` |
| `coupon_amount` | number/string | 优惠券面额 | 元 | 优惠券抵扣额 | 非空响应 + 组件 `prop` |
| `pay_amount` | number/string | 实付金额 | 元 | 客户实际支付金额 | 非空响应 + 组件 `prop` |
| `activity_cashback_amount` | number/string | 返现金额 | 元 | 活动返现金额 | 非空响应 + 组件 `prop` |
| `income_amount` | number/string | 收款金额 | 元 | 订单实际入账金额 | 非空响应 + 组件 `prop` |
...
| 日期范围 | `start_date` / `end_date` | `YYYY-MM-DD` | 当月第一天 / 当前日 | 下单日期范围 | 组件默认值 + 抓包 |
| 页码/页大小 | `page` / `limit` | integer | `1` / `20` | 分页 | 抓包 |

| 响应字段/路径 | 类型/可空 | 表格列 | 格式/单位 | 释义 | 证据 |
|---|---|---|---|---|---|
| `contact_name` | string | 下单客户 | 文本 | 客户名称 | 非空响应 + 组件 `prop` |
| `order_no` | string | 订单号 | 单号 | 积分兑换订单号 | 非空响应 + 组件 `prop` |
| `create_time` | string | 下单时间 | 日期时间 | 订单创建时间 | 非空响应 + 组件 `prop` |
| `status_text` | string | 订单状态 | 文本 | 状态显示名 | 非空响应 + 组件 `prop` |
| `name` | string | 商品名称 | 文本 | 兑换商品名称 | 非空响应 + 组件 `prop` |
| `number` | number/string | 商品件数 | 件 | 兑换数量 | 非空响应 + 组件 `prop` |
...
| 响应字段/路径 | 类型/可空 | 表格列 | 格式/单位 | 释义 | 证据 |
|---|---|---|---|---|---|
| `bill_date` | string | 单据日期 | 日期时间 | 申请单创建日期 | 非空响应 + 组件 `prop` |
| `bill_no` | string | 申请单号 | 单号 | 退货申请单号 | 非空响应 + 组件 `prop` |
| `customer_name` | string | 客户 | 文本 | 申请客户名称 | 非空响应 + 组件 `prop` |
| `total_amount` | number/string | 退货金额 | 元 | 申请退货总金额 | 非空响应 + 组件 `prop` |
| `total_qty` | number/string | 退货数量 | 件 | 申请退货总数量 | 非空响应 + 组件 `prop` |
| `statusName` | string | 退货状态 | 文本 | 状态显示名 | 非空响应 + 组件 `prop` |
| `saInvoiceNo` | string/可空 | 退货单号 | 单号 | 完成后关联的销售退货单号 | 非空响应 + 组件 `prop` |
| `remark` | string/可空 | 备注 | 文本 | 申请备注 | 非空响应 + 组件 `prop` |
...
| 发起时间 | `ctime_start` / `ctime_end` | datetime string | 空 | 发起日期范围 | 组件 watcher |
| 退款时间 | `rtime_start` / `rtime_end` | datetime string | 空 | 实际退款日期范围 | 组件 watcher |
| 数据来源 | `data_source` | number | `1` 小程序、`2` APP | 订单来源 | 组件枚举 |
| 页码/页大小 | `page` / `limit` | integer | `1` / `20` | 分页 | 抓包 |

| 响应字段/路径 | 类型/可空 | 表格列 | 格式/单位 | 释义 | 证据 |
|---|---|---|---|---|---|
| `bill_status_name` | string | 退款状态 | 文本 | 退款状态显示名 | 非空响应 + 组件 `prop` |
| `create_time` | string | 发起时间 | 日期时间 | 退款申请发起时间 | 非空响应 + 组件 `prop` |
| `bill_no` | string | 退款单号 | 单号 | 退款业务单号 | 非空响应 + 组件 `prop` |
| `sa_invoice_order_no` | string | 关联订单号 | 单号 | 被关闭/退款的订单号 | 非空响应 + 组件 `prop` |
| `order_type_name` | string/可空 | 关联订单来源 | 文本 | 关联订单类型显示名 | 非空响应 + 组件 `prop` |
| `contact_name` | string | 维修厂名称 | 文本 | 客户名称 | 非空响应 + 组件 `prop` |
| `refund_source` | string/可空 | 退款来源 | 文本 | 退款触发来源 | 非空响应 + 组件 `prop` |
| `pay_amount` | number/string | 退款金额 | 元 | 实际退款金额 | 非空响应 + 组件 `prop` |
| `pay_type_name` | string | 退款方式 | 文本 | 原路返回/线下退款显示名 | 非空响应 + 组件 `prop` |
| `out_bill_no` | string/可空 | 流水号 | 单号 | 外部支付退款流水号 | 非空响应 + 组件 `prop` |
| `nickname` | string/可空 | 退款人 | 文本 | 操作人员显示名 | 非空响应 + 组件 `prop` |
| `pay_success_time` | string/可空 | 退款时间 | 日期时间 | 退款成功时间 | 非空响应 + 组件 `prop` |
...
| 申请时间 | `create_beg_time` / `create_end_time` | date/datetime string | 空 | 调拨申请创建时间范围 | 查询对象 |
| 入库时间 | `in_beg_time` / `in_end_time` | date/datetime string | 空 | 调入完成时间范围 | 查询对象 |
| 出库时间 | `out_beg_time` / `out_end_time` | date/datetime string | 空 | 调出完成时间范围 | 查询对象 |
| 单号 | `bill_no` | string | 空 | 调拨申请单/关联调拨单号关键字 | 查询对象 |
| 页码/页大小 | `page` / `rows` | integer | `1` / `20` | 分页 | 抓包 |

响应外层为 `success:boolean`、`status:string`、`redirect:null`、`msg:string`、`data:object`；`data.records:number`、`data.total:number`、`data.list:array`、`data.total_in_price:number`、`data.total_out_price:number`、`data.statistics:object`。
...
响应外层：`success:boolean`、`status:string`、`redirect`、`msg:string`、`data.page:number`、`data.records:number`、`data.total:number`、`data.rows:array`。
...
| `buName` | null（该样本） | 兼容往来单位/供应商显示名，当前不展示 |
...
证据等级：B。页面“查询”产生的主列表 URL、全部默认参数、空响应外层和 jqGrid 列字段均已确认；当前日期范围内结果为空，因此行字段类型/可空性待补证。
...
当前响应为 `status:string`、`msg:string`、`data:array`，`data` 为空。组件列契约如下，类型/可空性均待非空响应确认：

| 响应字段 | 表格列 | 格式/单位 | 释义 |
|---|---|---|---|
| `billDate` | 配送日期 | 日期 | 退货/配送业务日期 |
| `billNo` | 订单编号 | 单号 | 大客户退货单号 |
| `serviceTypeName` | 业务类型（当前隐藏） | 文本 | 配送服务类型显示名 |
| `saleName` | 销售人员 | 文本 | 销售人员名称 |
| `delieverName` | 送货人员 | 文本 | 送货人员名称 |
| `billStatus` | 状态 | 状态显示 | 单据状态 |
| `transTypeName` | 业务类别 | 文本 | 退货业务类型显示名 |
| `contactName` | 客户 | 文本 | 客户名称 |
| `disRate` | 优惠率 | `%` | 整单优惠比例 |
| `disAmount` | 优惠金额 | 元 | 优惠金额 |
| `totalAmount` | 销售金额 | 元 | 关联销售/退货金额 |
| `postageKz` | 服务站配送费（当前隐藏） | 元 | 服务站配送费用 |
| `totalPurPrice` | 成本金额 | 元 | 商品成本金额 |
| `totalCost` | 毛利金额 | 元 | 毛利金额；不要仅凭字段名误作成本 |
| `userName` | 制单人 | 文本 | 制单人员名称 |
| `sourceType` | 来源 | 枚举显示 | 数据来源 |
| `srcChannelOrder` | 关联平台退单号 | 单号 | 平台退货单编号 |

## 大客户销售配送/退货明细

证据等级：配送明细 A，退货明细 B。两页主查询路径、重复 `transType` 参数、全部筛选项和列绑定均已确认；配送页有非空响应，退货页当前为空。
...
| 商品分类 | `categoryTreeAllValue` | string/collection | 空 | 分类树选择值 | 分类组件 + 抓包 |
...
| 分页/排序 | `rows` / `page` / `sidx` / `sord` | integer/integer/string/string | `1000000` / `1` / `billDate` / `desc` | 当前页面近似全量读取；Agent 应主动缩小日期范围 | grid 配置 + 抓包 |

响应外层为 `status:number`、`msg:string`、`data:object`，列表位于 `data.list[]`。配送页当前非空行字段：
...
| 时间 | `begin_date` / `end_date` | datetime string | 空 | 下单时间范围 | 日期范围控件 + 查询对象 |
| 页码/页大小 | `page` / `rows` | integer | `1` / `20` | 分页 | 抓包 + 分页组件 |

响应为 `code:number`、`msg:string`、`data.total:number`、`data.list[]`；组件以 `code=0` 判断成功。
...
| 时间 | `begin_date` / `end_date` | datetime string | 空；选中后默认时间为 `00:00:00` / `23:59:59` | 退单创建时间范围 | 日期范围组件 + 查询对象 |
| 页码/页大小 | `page` / `rows` | integer | `1` / `20`；UI 可选 `20/50/100` | 分页 | 抓包 + 分页组件 |

当前空响应为 `code:number`、`msg:string`、`data.total:number`、`data.list:[]`；组件以 `code=0` 判断成功。
...
证据等级：A。普通端点与“计算毛利”端点的真实查询、各 1711 行非空响应、查询处理器、jqGrid `colModel` 和页脚汇总绑定均已确认。
...
| UI 筛选项 | 请求参数 | 类型 | 默认值/枚举 | 释义 | 证据 |
|---|---|---|---|---|---|
| 日期 | `beginDate` / `endDate` | `YYYY-MM-DD` | 当前页面日期范围 | 销售业务日期起止 | UI + 查询处理器 + 抓包 |
...
| 三方类别 | `categoryTreeAllValue` | string/collection | 空 | 三方商品分类树选择值 | UI + 分类树组件 + 抓包 |
...
| jqGrid 控制 | `_search` / `nd` / `rows` / `page` / `sidx` / `sord` | boolean-like / timestamp / integer / integer / string / string | `false` / 动态 / `3000` / `1` / `date` / `desc` | 防缓存、近似全量读取和排序；Agent 应优先缩小日期范围 | 抓包 + jqGrid 配置 |

普通与 `_cost` 端点均返回 `status:number`、`data.rows:array`、`data.userdata:object`；当前真实响应没有 `records/total`，页面使用 `loadonce` 直接加载行数组。
...
证据等级：A。按商品、按客户两个页签的真实请求、非空响应、不同列表容器、动态仓库列和固定 `colModel` 均已确认。
...
| 分页/排序 | `rows` / `page` / `sidx` / `sord` | integer/integer/string/string | `1000000` / `1` / `date` / `desc` | 页面近似全量读取；Agent 应缩小日期范围 | grid 配置 + 抓包 |

两个端点的响应外层均为 `status:number`、`msg:string`、`data:object`，但列表容器不同：按商品是 `data.rows[]`，并带 `data.page:number`、`data.records:string`、`data.total:number`、`data.stoNames:array`、`data.userdata:object`；按客户是 `data.list[]`，并带 `data.total:object`。不能用同一个 `rows` 解析器处理两者。
...
响应外层为 `status:number`、`msg:string`、`data:object`。列表位于 `data.rows[]`，`data.colIndex[]` 与 `data.colNames[]` 定义动态金额列，`data.total` 给出合计；`data.stoNames` 当前为 string 兼容字段。
